...
function selectTriggeredSkills(skills = [], userMessage = '') {
  return skills
    .filter((skill) => skill?.active && (skill.content || skill.systemPromptFragment || skill.description))
    .filter((skill) => !skill.triggerKeywords?.length || triggerMatches(userMessage, skill.triggerKeywords));
}

function formatTriggeredSkills(skills = [], userMessage = '') {
  return selectTriggeredSkills(skills, userMessage)
      ...
      if (event.type === 'content_block_stop') {
        ...
        if (block?.type === 'tool_use' && block.partial_json) {
          try {
            ...
            block.input = null;
            block.inputError = 'Tool input JSON tidak valid.';
          }
...
async function runAgentLoop({ messages, maxLoops = 4, streamModel, executeTool, onText, onTool }) {
  ...
  for (let loop = 0; loop <= maxLoops; loop += 1) {
    ...
    if (!toolUses.length) throw new Error('Claude menghentikan respons untuk tool, tetapi tidak mengirim tool_use yang valid.');
    if (loop === maxLoops) throw new Error(`Agent melewati batas ${maxLoops} putaran tool.`);
    ...
    const toolResults = await Promise.all(toolUses.map(async (tool) => {
      ...
      if (tool.inputError || tool.input === null) {
        outcome = { content: tool.inputError || 'Tool input tidak valid.', isError: true };
      } else {
        try {
          outcome = { content: await executeTool(tool), isError: false };
        } catch (error) {
          outcome = { content: error.message || 'Tool gagal dijalankan.', isError: true };
        }
      }
  ...
  throw new Error('Agent loop berhenti tanpa hasil final.');
}
  ...
  for (let loop = 0; loop <= maxLoops; loop += 1) {
    ...
    const toolResults = await Promise.all(toolUses.map(async (tool) => {
      try {
        ...
        return { type: 'tool_result', tool_use_id: tool.id, content: error.message || 'Tool gagal dijalankan.', is_error: true };
      }
    }));
    workingMessages.push({ role: 'user', content: toolResults });
  }
